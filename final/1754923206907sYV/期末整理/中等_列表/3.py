...
The minimum gap in absolute value between successive elements in L is: 0"""
            ...
            print(
                f"""The second smallest element in L is: {max(L[0],L[1])}
The minimum gap in absolute value between successive elements in L is: {abs(L[1]-L[0])}"""
            )
    else:
        a = L.copy()
        a.sort()
        ...
        for i in range(1, len(L) - 1):
            if mingap > abs(L[i + 1] - L[i]):
                mingap = abs(L[i + 1] - L[i])
        print(
            f"""The second smallest element in L is: {min}
The minimum gap in absolute value between successive elements in L is: {mingap}"""
        )
