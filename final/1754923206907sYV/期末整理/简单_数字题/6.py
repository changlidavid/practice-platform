    ...
    for i in range(a, b + 1):
        for j in range(i, b + 1):
            if (i + j) % 2 == 0:
                res = (i + j) // 2
                si = list(str(i))
                sorted_i = sorted(list(si))
                set_i = set(si)

                sj = list(str(j))
                sorted_j = sorted(list(sj))
                set_j = set(sj)
                
                sres = list(str(res))
                sorted_res = sorted(list(sres))
                set_res = set(sres)
                
                if (
                    len(si) == len(set_i)
                    and len(sj) == len(set_j)
                    and len(sres) == len(set_res)
                ):
                    if (
                        si == sorted_i
                        and sj[::-1] == sorted_j
                        and (sres == sorted_res or sres[::-1] == sorted_res)
                    ):
                        print(f"{i} and {j} with {res} as average")
