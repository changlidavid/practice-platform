...
def cross(for_seed, size):
    ...
    for c in range(1, size - 1):
        for r in range(1, size - 1):
            temp = grid[c][r] + grid[c - 1][r - 1] + grid[c + 1][r + 1] + grid[c - 1][r + 1] + grid[c + 1][r - 1]
            array.append(temp)

    array.sort(reverse=True)
    if array:
        if array[0] == array[-1]:
            nb_of_crosses_of_max_value = len(array)
        else:
            nb_of_crosses_of_max_value = 1
            for num in range(len(array)):
                if array[num] != array[num + 1]:
                    break
                nb_of_crosses_of_max_value = nb_of_crosses_of_max_value + 1
        max_value = array[0]
