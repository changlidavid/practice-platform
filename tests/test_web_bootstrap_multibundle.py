...
def test_create_app_prunes_bundles_removed_from_bundle_paths(
    ...
    monkeypatch.setattr(web, "load_env_file", lambda repo_root: None)
    ...
    try:
        ...
        count_before = len(slugs_before)
        assert "sample_1" in slugs_before
        assert any(slug.startswith("final:") for slug in slugs_before)
        assert "problems:average_of_digits_common_to" in slugs_before
    finally:
        conn.close()
    ...
    try:
        ...
        assert int(count_after["c"]) == pruned_count
        assert int(count_after["c"]) < count_before
        assert int(count_after["c"]) >= 10
    finally:
        conn.close()
